...
my_gamma = 0.92 # 0.74, 0.76, 0.78, 0.80, 0.82, 0.84, 0.86, 0.88, 0.90, 0.92
c = 0.9
part_numer = 0.5
def bcq_operator(Q, mdp, mu, mu_hat, zeta, noise, *args):
    ...
    """
    Q = (Q + (np.random.rand(mdp.N_S, mdp.N_A) - 0.5) * 2.0 * noise).clip(0, 10 / (1 - my_gamma))
    
    Q_update = mdp.r + my_gamma * np.dot(mdp.P, Q.max(-1))
...
for _ in range(100):
    ...
    for part in (part_numer, ): # 0.5, 0.51, 0.52, 0.53, 0.54, 0.55,
        for noise in (0.1,):
...
for part in (part_numer,): # 0.55, 0.6, 0.7, 0.8, 0.9
    data = res_bcq_6[(res_bcq_6['noise']==noise) & (res_bcq_6['part']==part)]['error']
    print(f"error: {data.mean():.2f}  c: {c}  gamma: {my_gamma}")